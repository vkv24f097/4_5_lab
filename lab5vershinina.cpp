// lab5vershinina.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
// Функция для преобразования строки в нижний регистр для упрощения поиска
...
// Сортировка слиянием для поиска по убыванию частоты
...
int main() {
    // Хранилище для слова 
    unordered_map<string, vector<WordInfo>> index; //в каких документах и сколько раз встречается
    // Хранилище для пар слов
    unordered_map<string, vector<WordInfo>> pair_index; //то же саммое для пары слов
    ...
    for (int i = 0; i < num_docs; i++) {
        ...
        // Индексация слов и пар слов
        stringstream ss(documents[i]);//разбиваем текст на слова
        ...
        while (ss >> word) {
            word = to_lower(clean_word(word)); //очищаем и приводим к единому регистру
            if (!word.empty()) {
                word_freq[word]++;
